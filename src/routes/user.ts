...
import {
  getAllOpenBets, getBet, createPosition, updatePools,
  getUserPositions, upsertUser, getUser, getPosition
} from '../db';
import { buildPaymentLink, computeOdds, isValidTonAddress, getAdminAddress } from '../ton';
...
router.get('/me', (req, res) => {
  const tgUser: TelegramUser = (req as any).telegramUser;
  upsertUser(tgUser.id, tgUser.username);
...
router.post('/wallet', (req, res) => {
  const tgUser: TelegramUser = (req as any).telegramUser;
  const { ton_address } = req.body as { ton_address: string };

  if (!ton_address || !isValidTonAddress(ton_address)) {
    return res.status(400).json({ error: 'Invalid TON address' });
  }

  upsertUser(tgUser.id, tgUser.username, ton_address);
  res.json({ ok: true });
});
...
router.post('/positions', async (req, res) => {
  const tgUser: TelegramUser = (req as any).telegramUser;
  const { bet_id, side, amount_ton } = req.body as {
    bet_id: number; side: 'yes' | 'no'; amount_ton: number;
  };
  ...
  const bet = getBet(bet_id);
  if (!bet) return res.status(404).json({ error: 'Bet not found' });
  if (bet.status !== 'open') return res.status(409).json({ error: 'Bet is not open' });
  ...
  const positionId = createPosition(
    bet_id, tgUser.id, tgUser.username,
    user.ton_address, side, amount_ton, expiresAt
  );

  // Optimistically add to pool — reverted automatically if payment never arrives
  updatePools(bet_id, side, amount_ton);

  const adminAddress = await getAdminAddress();
  const paymentLink = buildPaymentLink(adminAddress, amount_ton, bet_id, side, positionId);
...
router.get('/positions', (req, res) => {
  const tgUser: TelegramUser = (req as any).telegramUser;
  const positions = getUserPositions(tgUser.id);
  res.json(positions);
});
...
router.get('/positions/:id', (req, res) => {
  const tgUser: TelegramUser = (req as any).telegramUser;
  const position = getPosition(parseInt(req.params.id));
  if (!position) return res.status(404).json({ error: 'Position not found' });
  if (position.user_id !== tgUser.id) return res.status(403).json({ error: 'Forbidden' });
  res.json(position);
});
